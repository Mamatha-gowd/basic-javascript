...
const makeCat = (name, age) => {
  ...
  newCat.name = name;
  newCat.age = age;
  newCat.meow = function () { return 'Meow!'; };
  return newCat;
};
...
const multiplyMysteryNumberByFive = (mysteryNumberObject) => {
  ...
  const pdt = mysteryNumberObject.mysteryNumber * 5;
  return pdt;
};
...
const setUsersToPremium = (users) => {
  ...
  users.map((user) => { return user.isPremium = true; });
  return users;
};
...
const sumUserPostLikes = (user) => {
  ...
  // return the sum
  const sum = user.posts.map((post) => { return post.likes; }).reduce((acc, red) => { return acc += red; });
  return sum;
};
